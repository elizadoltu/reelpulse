...
describe('analyticsProcessor handler shape', () => {
  it('calls res.status(200).json with ok payload', async () => {
    ...
    await (async (_req: typeof req, r: typeof res) => {
      r.status(200).json({ status: 'ok', function: 'cf-analytics' });
    })(req, res);

    expect(status).toHaveBeenCalledWith(200);
    expect(json).toHaveBeenCalledWith({ status: 'ok', function: 'cf-analytics' });
  });
});
