...
const API_ENDPOINTS = {
  ...
  MOVIE_REVIEW_STATUS: '/movies/:movie_id/reviews/:review_id/status',
} as const;
...
const MOVIE_REVIEW_STATUS_ENDPOINT = (movieId: string, reviewId: string): string =>
  API_ENDPOINTS.MOVIE_REVIEW_STATUS.replace(':movie_id', movieId).replace(':review_id', reviewId);
